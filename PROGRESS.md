...
## Сессия: расписания трансферов, галерея автопарка, /info → /helpful, аудит ISR

- [x] П.5 Публичное расписание трансферов: чинился скрытый баг — легаси-порядок секций
  (сохранённый до появления "schedules") выкидывал расписания из рендера. Логика вынесена
  в `resolveTransferSectionOrder` (lib/transfer-display.ts): недостающие секции из
  TRANSFER_SECTION_DEFAULT_ORDER дополняются в конец. Тест tests/transfer-display.test.ts.
- [x] П.9 Автопарк, галерея: bus-base-form не подхватывал замену/удаление фото — форма
  теперь синхронизирует состояние галереи с draft-coordinator и submit-ом.
- [x] /info → /helpful: каталог app/(site)/info переименован в helpful, все внутренние
  ссылки заменены (app/components/lib/scripts), 301-редиректы 1:1 в next.config.mjs
  (`/info` и `/info/:path*` → permanent). `helpful` добавлен в резервные слаги avia-slug.
...
- Весь публичный сайт работает в режиме SSR без кэша: `export const dynamic = "force-dynamic"`
  стоит в `app/(site)/layout.tsx` и на всех публичных страницах (с первого импорта проекта).
  Layout сам читает настройки из БД на каждый запрос, так что ISR отдельных страниц
  без переработки layout невозможен.
- Все вызовы `revalidatePath()` в admin server actions (~200 шт., включая lib/admin-action.ts
  и lib/revalidate-catalog.ts) при force-dynamic фактически no-op — вреда нет, но и кэша,
  который они бы сбрасывали, нет. Оставлены как задел на будущее.
- Замеры на локальном стенде: повторный TTFB ~0.1 с на всех публичных маршрутах —
  СУБД на том же хосте, что и на проде (локальный PostgreSQL на VPS), т.е. цена
  force-dynamic приемлема.
- Переход на ISR сейчас рискован и не рекомендуется: детальные страницы принимают
  preview-токены через searchParams (комментарий в туре: «static shell даёт
  DYNAMIC_SERVER_USAGE 500»), а CMS-модель «каждый текст из БД» требует ревалидации
  layout-поддерева на каждое сохранение настроек. Если потребуется ускорение —
  начинать с кэширования запросов (React cache уже используется в lib/cms.ts) и
  `unstable_cache`/use cache для тяжёлых выборок, а не с page-level ISR.
