...
import { aviaHomePageConfig, resortsSectionFields } from "@/lib/admin-config"
...
import { adminAviaHomeHref } from "@/lib/admin-public-href"

export const metadata: Metadata = { title: "Авиатуры — Главная — Админ-панель" }

const DEFAULT_ORDER = [...DESTINATION_DEFAULT_SECTION_ORDER]
const STATIC_HEADINGS = ["Шапка страницы"]

export default async function AviatoryHomePage() {
  const pageKey = "aviatory"
  const page = aviaHomePageConfig()
  ...
  const visibleCities = allCities.filter((c) => settings[`city:avia:${c.slug}.visible`] !== "0")

  // Pre-render seo* slots
  ...
  return (
    <div className="space-y-4">
      <PageSettingsForm
        title={page.heading}
        description="SEO, шапка страницы, карточки курортов и FAQ."
        pageHref={adminAviaHomeHref(settings["aviatory.slug"])}
        visibilityKey="aviatory.visible"
        defaultVisible={settings["aviatory.visible"] !== "0"}
        ...
        workspaceExtraPanels={[
          ...
          <div key="resort-table" id="resort-table" className="mt-6 scroll-mt-4">
              <ResortTableBuilder
                pageKey={pageKey}
                blocks={resortBlocks}
                returnTo="/admin/pages/aviatory-home"
              />
          </div>,
        ]}
        workspaceAfterForm={
              ...
              search: searchGroup ? (
                <div className="space-y-4">
                  <p className="text-sm text-admin-fg-muted">
                    Фильтры и список авиатур в каталоге. Перетащите секцию, чтобы разместить
                    остальные блоки выше или ниже каталога.
                  </p>
                  <SectionFieldsForm fields={searchGroup.fields} settings={settings} hideSubmit />
                </div>
              ) : (
                <p className="text-sm text-admin-fg-muted">
                  Фильтры и список авиатур в каталоге.
                </p>
              ),
              cities: citiesGroup ? (
                <>
                <p className="mb-4 text-sm text-admin-fg-muted">На сайте показываются курорты этой страницы, отмеченные как видимые; скрытые не отображаются. Управляйте видимостью в разделе «Курорты». Сейчас видимых на сайте: {visibleCities.length} из {allCities.length}.</p>
