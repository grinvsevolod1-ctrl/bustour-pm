...
export const metadata: Metadata = { title: "Горящие туры — Главная — Админ-панель" }

const DEFAULT_ORDER = [...DESTINATION_DEFAULT_SECTION_ORDER]
const STATIC_HEADINGS = ["Шапка страницы"]

export default async function HotHomePage() {
  const pageKey = "hot"
  const page = hotHomePageConfig()
  ...
  return (
    <div className="space-y-4">
      <PageSettingsForm
        ...
        pageHref={pageHref}
        visibilityKey="hot.visible"
        defaultVisible={settings["hot.visible"] !== "0"}
              ...
              search: searchGroup ? (
                <div className="space-y-4">
                  <p className="text-sm text-admin-fg-muted">
                    Фильтры и каталог горящих туров (подбор по цене, дате, ночи).
                    Перетащите секцию, чтобы разместить блоки выше или ниже каталога.
                  </p>
                  <SectionFieldsForm fields={searchGroup.fields} settings={settings} hideSubmit />
                </div>
              ) : (
                <p className="text-sm text-admin-fg-muted">
                  Фильтры и каталог горящих туров.
                </p>
              ),
              cities: citiesGroup ? (
