...
4. **sharp** на сервере собран из исходников под CPU хоста; `npm ci` в
   ...
   Обновлён до 0.35.x (закрыты CVE libvips). После первого деплоя с этой
   версией проверь `pm2 logs bastur-media-worker` и загрузку картинки в
   админке — если sharp не собрался, `npm rebuild sharp` на сервере.
